...
{	
	...
	public:
		...
		explicit					TriggeringTimer(float timeLimit, bool initiallyRunning = false);
		
		/// @brief Resets the Timer's countdown to the given time limit.
		/// @param timeLimit The new time (must be greater than zero).
		/// @param continueRunning Indicates whether the timer just continues.
		...
		/// @details Make sure to call Check() each frame to invoke potential listeners.
