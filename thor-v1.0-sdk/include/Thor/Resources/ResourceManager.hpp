...
///   @arg <b>thor::MovedPtr<Resource> ResourceKey::Load() const</b> loads the resource and returns a smart pointer to it
...
class ResourceManager : private NonCopyable
{
	...
	public:
		...
		/// @brief Destructor
		/// @details Destroys all allocated resources. All ResourcePtr instances pointing to any of the resources stored
		///  inside this ResourceManager become @a NULL.
									~ResourceManager();
	...
	private:
		...
		typedef detail::ResourceSlot<MutableResource, ResourceKey>	ResourceSlot;
	...
	private:
		// Loads and inserts a resource. Sets itr to the inserted resource.
		ResourcePtr<Resource>		AddResource(SlotIterator itr, const ResourceKey& key);
	
		// Unloads and releases a resource.
		// If considerAutoRelease is true, the resource is only released if auto-release is enabled.
	...
	// ---------------------------------------------------------------------------------------------------------------------------
	// Friends
	friend class detail::ResourceSlot<MutableResource, ResourceKey>;
};
